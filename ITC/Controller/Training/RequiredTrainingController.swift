//
//  RequiredTrainingController.swift
//  ITC
//
...
class RequiredTrainingController: UIViewController,UITableViewDelegate,UITableViewDataSource  {

    @IBOutlet weak var requiredTrainingTableView: UITableView!
    @IBOutlet weak var noRecordsFoundLabel: UILabel!
   
    
    var vw = UIView()
    var requiredArray: [REquiredTraining]?
    ...
    override func viewDidLoad() {
        ...
        if(UserProfileDefaults.isEnglish){
            titleView.PageTitleLabel.text = "Required Training"
        }else{
            titleView.PageTitleLabel.text = NSLocalizedString("Required Training", comment: "")
        }
        ...
        TrainingAPI.sharedInstance.fetchRequiredTraining { (requiredArray, isCompleted) in
            if(isCompleted){
                self.requiredArray = requiredArray
                self.removeActivityIndicatorView()
                self.requiredTrainingTableView.reloadData()
                self.noRecordsFoundLabel.text = ""
            }else{
                self.removeActivityIndicatorView()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cellId", for: indexPath) as! RequiredTrainingCell
        if let array = requiredArray {
